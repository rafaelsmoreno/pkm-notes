  ...
  (let [document-mode? (or (storage/get :document/mode?) false)
    ...
    (atom
     {:client-id                             (str (random-uuid))
      ...
      ;; read-only (notes-folder) mode per graph: no writes under graph root
      :graph-read-only                  (or (storage/get :graph-read-only) {})
...
(defn get-graph-read-only?
  [repo-url]
  (boolean (get-in @state [:graph-read-only repo-url])))

(defn set-graph-read-only!
  [repo-url read-only?]
  (when repo-url
    (swap! state update :graph-read-only assoc repo-url (boolean read-only?))
    (storage/set :graph-read-only (get @state :graph-read-only))))
