...
(defn- paths->tree
  "Build a nested tree from file paths. Paths are strings like 'a/b/c.md'.
   Returns {:children {\"a\" {:children {\"b\" {:children {\"c.md\" :file}}}}}}."
  [paths]
  (reduce
   (fn [tree path]
     (let [parts (string/split path #"/")
           _ (assert (seq parts) (str "Empty path parts for " path))
           path-keys (concat (mapcat (fn [p] [:children p]) (butlast parts))
                             [:children (last parts)])]
       (assoc-in tree path-keys :file)))
   {}
   paths))

(defn- file-tree-node < rum/reactive
  [path-prefix node depth]
  (if (identical? node :file)
    (let [path (str path-prefix)]
      [:a.file-tree-file.block.font-medium
       {:key path
        :href (rfe/href :file {:path path})
        :class "text-sm py-0.5 pl-2 hover:opacity-80"}
       (last (string/split path #"/"))])
    (let [children (sort-by first (:children node))
          expanded-key (str "file-tree-" path-prefix)
          expanded? (state/sub [:ui/navigation-item-collapsed? expanded-key])
          expanded? (not expanded?)]
      [:div.file-tree-folder {:key path-prefix}
       (for [[name child] children]
         (let [full-path (if (string/blank? path-prefix) name (str path-prefix "/" name))]
           (if (identical? child :file)
             [:a.file-tree-file.block.font-medium
              {:key full-path
               :href (rfe/href :file {:path full-path})
               :class "text-sm py-0.5 pl-2 hover:opacity-80"
               :style {:padding-left (str (+ 8 (* 12 depth)) "px")}}
              name]
             (let [expanded-key* (str "file-tree-" full-path)
                   collapsed? (state/sub [:ui/navigation-item-collapsed? expanded-key*])]
               [:div.file-tree-folder-item {:key full-path}
                [:div.sidebar-content-group.hd.items-center.cursor-pointer.select-none
                 {:class (util/classnames [{:is-expand (not collapsed?)}])
                  :on-click #(state/toggle-navigation-item-collapsed! expanded-key*)
                  :style {:padding-left (str (+ 4 (* 12 depth)) "px")}}
                 [:span.a (shui/tabler-icon (if collapsed? "chevron-right" "chevron-down") {:size 14})]
                 [:span.b.text-sm name]]
                (when-not collapsed?
                  [:div.bd (file-tree-node full-path child (inc depth))])]))))])))

(rum/defcs sidebar-file-tree < rum/reactive
  {:init (fn [state]
           (let [*files (atom nil)]
             (when-let [repo (state/get-current-repo)]
               (p/let [result (db-async/<get-files repo)]
                 (reset! *files (or (seq result) []))))
             (assoc state ::file-list *files)))}
  [state]
  (let [files (rum/react (::file-list state))
        paths (when files (map first files))
        tree (when (seq paths) (paths->tree paths))
        route-match (state/sub :route-match)
        current-path (get-in route-match [:parameters :path :path])]
    (sidebar-content-group
     [:a.wrap-th [:strong.flex-1 (t :left-side-bar/file-tree)]]
     {:class "file-tree"
      :count (count paths)}
     (when (seq (get tree :children))
       [:div.file-tree-root.text-sm
        (for [[name child] (sort-by first (get tree :children))]
          (if (identical? child :file)
            [:a.file-tree-file.block.font-medium
             {:key name
              :href (rfe/href :file {:path name})
              :class (util/classnames ["py-0.5 pl-2 hover:opacity-80"
                                      {:active (= current-path name)}])}
             name]
            (let [expanded-key (str "file-tree-" name)
                  collapsed? (state/sub [:ui/navigation-item-collapsed? expanded-key])]
              [:div.file-tree-folder-item {:key name}
               [:div.sidebar-content-group.hd.items-center.cursor-pointer.select-none
                {:class (util/classnames [{:is-expand (not collapsed?)}])
                 :on-click #(state/toggle-navigation-item-collapsed! expanded-key)}
                [:span.a (shui/tabler-icon (if collapsed? "chevron-right" "chevron-down") {:size 14})]
                [:span.b name]]
               (when-not collapsed?
                 [:div.bd (file-tree-node name child 1)])])))]))))
