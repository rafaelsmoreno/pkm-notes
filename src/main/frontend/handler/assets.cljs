...
(defn clean-path-prefix
  [path]
  (when (string? path)
    (string/replace-first path (re-pattern (str "^[.\\/\\\\]*(?:" (string/replace common-config/local-assets-dir "/" "[\\/\\\\]") ")[\\/\\\\]+")) "")))
    ...
    (p/let [result (p/catch (fs/readdir path {:path-only? true})
      ...
      (p/all (map (fn [path]
                    (p/let [data (fs/read-file-raw path "" {})]
                      (let [path' (util/node-path.join common-config/local-assets-dir (util/node-path.basename path))]
                        [path' data]))) result)))))

(defn ensure-assets-dir!
  [repo]
  (p/let [repo-dir (config/get-repo-dir repo)
          assets-dir common-config/local-assets-dir
          _ (when-not (config/graph-read-only? repo)
              (fs/mkdir-if-not-exists (path/path-join repo-dir assets-dir)))]
    [repo-dir assets-dir]))
